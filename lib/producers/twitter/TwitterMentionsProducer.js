...
function TwitterMentionsProducer() {
	base.init( this );
...
TwitterMentionsProducer.prototype.attemptRequest = function( uri, owner, source, resource, keys, callback ) {
	var self = this;
...
TwitterMentionsProducer.prototype.getSomeTweetsPriorTo = function( maxId, userId, keys, callback ) {
	var url;
	if( maxId != null )
		url = 'https://api.twitter.com/1.1/statuses/mentions_timeline.json?count=200&include_rts=1&user_id=' + userId + '&max_id=' + maxId;
	else
		url = 'https://api.twitter.com/1.1/statuses/mentions_timeline.json?count=200&include_rts=1&user_id=' + userId;
...
TwitterMentionsProducer.prototype.getUrlUsingOauth = function( url, keys, callback ) {
	var self = this;
	var retryCount = 0;
