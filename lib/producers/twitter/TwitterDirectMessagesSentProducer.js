...
function TwitterDirectMessagesSentProducer() {
	base.init( this );
...
TwitterDirectMessagesSentProducer.prototype.attemptRequest = function( uri, owner, source, resource, keys, callback ) {
	var self = this;
...
TwitterDirectMessagesSentProducer.prototype.getSomeTweetsPriorTo = function( maxId, userId, keys, callback ) {
	var url;
	if( maxId != null )
		url = 'https://api.twitter.com/1.1/direct_messages/sent.json?count=200&user_id=' + userId + '&max_id=' + maxId;
	else
		url = 'https://api.twitter.com/1.1/direct_messages/sent.json?count=200&user_id=' + userId;
...
TwitterDirectMessagesSentProducer.prototype.getUrlUsingOauth = function( url, keys, callback ) {
	var self = this;
	var retryCount = 0;
