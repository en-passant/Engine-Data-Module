...
ImapService.prototype.shutdown = function( callback ) {
	...
	Async.each( toRemove,
		...
		function( error ) {
			if( error )
				SystemLog.log( 'Error closing IMAP connections', error );
							...
							function( error, message ) {
								...
								{	
									connection.closeBox( function( error ) {
										...
										{
											self._disposeOfDeadConnection( owner, account, connection );
											SystemLog.log( 'Error closing IMAP mailbox', error );
	...
	self._getConnectionForAccount( owner, account, function( error, connection ) {
		...
		else
		{
			if( connection )
			{
				var deadListener = function() {
					connection.deadListener = null;
					SystemLog.error("Error retrieving search results for IMAP connection: (" + connection._options.username + ") ", error);
					callback( new Error( 'Connection was killed' ), null );
				}
				connection.deadListener = deadListener;
...
ImapService.prototype._encodeXOAuth2 = function( username, accessToken ) {
	SystemLog.log( '*** Encoding XOAuth2, username: ' + username + ', accessToken: ' + accessToken );
...
ImapService.prototype._refreshAndCreateConnection = function( owner, account, callback ) {
	...
	this._refreshToken( owner, account, function( error, newConnectionData ) {
		if( error )
		{
			...
			//SystemLog.error( 'Error in token refresh.', error );
			callback( error, null );			
		}
		else
		{
			...
			SystemLog.log( '(2)    Connecting again using ' + newConnectionData.accessToken );
			connection.connect( function( error ) {
				if( error )
				{
					// Don't need to log here because this is an internal function and we're passing the error up.
					//SystemLog.error( 'Error in reconnect.', error );
					callback( error, null );
				}
				else
				{
...
ImapService.prototype._createNewConnection = function( owner, account, callback ) {
	...
	else // If we're doing oauth instead,
	{
		    ...
		    connection.connect( function( error ) {
		        if( error && error.status != "400" )
		        {
		        	if( self._isFatalConnectionError( error ) )
		        	{
		        		...
		        		{
		        			// Don't need to log here because this is an internal function and we're passing the error up.
							//SystemLog.error( 'Authentication Error, no refreshToken was available.', error );
		        			callback( error, null );
		        		}
		        	}
		        	if( encodedAccountInfo.refreshToken && self._isDueToBeRefreshed( owner, account ) )
		        	{
		        		SystemLog.log( 'Refreshing IMAP access token due to error.  When an error occurs and this connection is not due to be refreshed, we\'ll log it.' );
		        		self._refreshAndCreateConnection( owner, account, callback );
		        	}
		        	else
		        	{
		        		// Don't need to log here because this is an internal function and we're passing the error up.
						//SystemLog.error( 'Error making IMAP connection, and no refreshToken was available', error );
		            	callback( error, null );        		
		        	}
		        }
		        else
		...
		else // If we didn't have an access token, let's just try to refresh.
		{
        	...
        	{
        		SystemLog.log( 'No access token was available for IMAP connection, attempting to refresh.  Account: ' + require( 'util' ).inspect( account ) );				
        		self._refreshAndCreateConnection( owner, account, callback );
        	}
        	else
        	{
...
ImapService.prototype._addConnectionErrorHandler = function( connection ) {
	connection.on( 'error', function( error ) {
		SystemLog.log( 'Error in IMAP connection', error );
	});
}
...
ImapService.prototype._deleteOldRefreshTimes = function() {
	...
	Async.eachSeries( 
		...
		function( error ) {
			...
			SystemLog.log( 'Old Refreshed Time Cleanup completed (non-blocking) for ' + connectionCount + ' connections in ' + ( endTime - startTime ) + 'ms.' );
...
ImapService.prototype._deleteIdleConnections = function() {
	...
	Async.each( toCheck,
		function( connectionInfo, done ) {
			...
			{
				SystemLog.log( 'Removing idle IMAP connection for ' + connectionInfo.owner + ', ' + connectionInfo.account.host + ', ' + connectionInfo.account.username );
				self._disposeOfDeadConnection( 
					connectionInfo.owner, 
					connectionInfo.account,
					connectionInfo.connection,
		...
		function( error ) {
			if( error )
				SystemLog.log( 'Error closing idle IMAP connections', error );
			setTimeout( function() {
...
ImapService.prototype._refreshToken = function( owner, account, done ) {
	...
	oauth2.getOAuthAccessToken( 
		...
		function( error, accessToken, refreshToken, results ) {
			...
			{
				SystemLog.log( 'Refreshed access token without error'/*!  refreshToken was: ' + account.refreshToken + ', clientId: ' + engine.config.gmail.clientID + ', clientSecret: ' + engine.config.gmail.clientSecret*/ + ', new accessToken: ' + accessToken );
