			...
			function( data, done ) {
    			try
				{
					...
					NLPService.addAnalysisIfMissing( outputData, function( error, result ) {
						try
						{
				   			...
				   			else
				   			{
								...
								NLPService.addAnalysisIfMissing( outputData, function( error, result ) {
									...
									{
										SystemLog.error( 'Error translating message: ' + require( 'util' ).inspect( data ), error );
										done( error );
									}
						...
						{
							SystemLog.error( 'Error translating recipient: ' + require( 'util' ).inspect( data ), error );
							done( error );
						}
			   		} );
				}
				...
				{
					SystemLog.error( 'Error translating sender: ' + require( 'util' ).inspect( contact ), error );
					done( error );
				}
