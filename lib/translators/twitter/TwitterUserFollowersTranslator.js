...
TwitterUserFollowersTranslator.prototype.translate = function( uri, owner, rawDoc, callback ) {
  ...
  //console.log("TwitterUserFollowersTranslator : rawDoc 123:" + JSON.stringify(rawDoc));
  var self = this;
  try {
    var followersData = rawDoc.data;//JSON.parse(rawDoc.data).data;
    //console.log("TwitterUserFollowersTranslator :  followersData:" + JSON.stringify(followersData));
    var parsedUri = uri.match( /([^:]*):\/\/(.*)\/\/@([^\/]*)(.*)/ );
    var source = parsedUri[3];
    async.forEachSeries(followersData.users, function(follower, callback_s1) {
      //console.log("TwitterUserFollowersTranslator :  follower:" + JSON.stringify(follower));
