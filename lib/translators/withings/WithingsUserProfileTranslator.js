...
function WithingsUserProfileTranslator() {
}

WithingsUserProfileTranslator.prototype.getMatchPatterns = function() {
    return [ '^acct:withings:[0-9]+', '/user/[0-9]+' ];
}
WithingsUserProfileTranslator.prototype.translate = function( uri, owner, rawDoc, callback ) {
    try {
        ...
        var result = { 
            ...
            'id': uri,
            'displayName': {'formatted': userProfile[0].data.firstname + ' ' + userProfile[0].data.lastname },

            // Additional required fields for UI
            'preferredUsername': userProfile[0].data.firstname + ' ' + userProfile[0].data.lastname,
            'thumbnailUrl': '/images/generic_user_image.jpeg',
            'appData': { 
                'serviceName': 'Withings',
                // Reuse the icon you defined in LoginStrategies.js.
                'serviceImgUrl': '/images/512x512-logos/withings.png',
...
module.exports = exports = WithingsUserProfileTranslator;
