...
function KloutUserProfileTranslator() {
}

KloutUserProfileTranslator.prototype.getMatchPatterns = function() {
    return [ '^acct:klout:[0-9]+', '/user/[0-9]+' ];
}
KloutUserProfileTranslator.prototype.translate = function( uri, owner, rawDoc, callback ) {
	var userProfile = JSON.parse( rawDoc.data ),
		parseAccountId = uri.match( /acct:klout:[0-9]+/ ),
	...
	async.eachSeries( 
		...
		function( each, done ) {
			if( userProfile.hasOwnProperty( each ) && parseId[0] ==  userProfile[each].data.kloutId )
			{
				try {
					...
					var result = { 
						...
						'id': uri,
						'displayName': {'formatted': userProfile[each].data.nick },

						// Additional required fields for UI
						'preferredUsername': userProfile[each].data.nick,
						'thumbnailUrl': '/images/generic_user_image.jpeg',
						'appData': { 
							'serviceName': 'Klout',
							// Reuse the icon you defined in LoginStrategies.js.
							'serviceImgUrl': '/images/512x512-logos/klout.png',
...
module.exports = exports = KloutUserProfileTranslator;
