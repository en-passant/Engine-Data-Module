...
function SoundcloudUserProfileTranslator() {
}

SoundcloudUserProfileTranslator.prototype.getMatchPatterns = function() {
    return [ '^acct:soundcloud:[0-9]+', '/user/[0-9]+' ];
}
SoundcloudUserProfileTranslator.prototype.translate = function( uri, owner, rawDoc, callback ) {
	var userProfile = JSON.parse( rawDoc.data ),
		parseAccountId = uri.match( /acct:soundcloud:[0-9]+/ ),
		...
		function( each, done ) {
			...
			try {
				...
				var result = { 
					...
					'id': uri,
					'displayName': {'formatted': userProfile[each].data.full_name },
					...
					'appData': { 
						...
						'serviceImgUrl': '/images/512x512-logos/soundcloud.png',
					...
					'aboutMe': userProfile[each].data.description,
					'urls': [ userProfile[each].data.permalink_url ],
					'utcOffset': 'null',
...
module.exports = exports = SoundcloudUserProfileTranslator;
