...
function TumblrNewsFeedTranslator() { 
}

TumblrNewsFeedTranslator.prototype.getMatchPatterns = function() {
    return [ '^acct:tumblr:[A-Za-z0-9\-]+', '.*/fnews' ];
}
TumblrNewsFeedTranslator.prototype.translate = function( sourceUri, owner, rawDoc, callback ) {
   try {
        ...
        async.eachSeries(
        	...
        	function( post, done ) {
    			try
				{
					...
					var outputData = {
						'sourceUri': sourceUri,
						'uri': 'ldengine://' + owner + '//@' + source + '/fnews/' + postsData[i].data.id,
						'owner': owner,
						'category': 'message',
...
TumblrNewsFeedTranslator.prototype.translateMessage = function( owner, source, rawMessage ) {
	return {
...
module.exports = exports = TumblrNewsFeedTranslator;
