...
function ImgurUserProfileTranslator() {
}

ImgurUserProfileTranslator.prototype.getMatchPatterns = function() {
    return [ '^acct:imgur:[A-Za-z0-9]+', '/user/[A-Za-z0-9]+' ];
}
ImgurUserProfileTranslator.prototype.translate = function( uri, owner, rawDoc, callback ) {
	var userProfile = JSON.parse( rawDoc.data ),
		parseAccountId = uri.match( /acct:imgur:[A-Za-z0-9]+/ ),
	...
	async.eachSeries( 
		...
		function( each, done ) {
			if( userProfile.hasOwnProperty( each ) && parseId[2] ==  userProfile[each].data.url ) {
			...
			try {
				...
				var result = { 
					...
					'id': uri,
					'displayName': {'formatted': userProfile[each].data.url },

					// Additional required fields for UI
					'preferredUsername': userProfile[each].data.url,
					'thumbnailUrl': '/images/generic_user_image.jpeg',
					'appData': { 
						'serviceName': 'Imgur',
						// Reuse the icon you defined in LoginStrategies.js.
						'serviceImgUrl': '/images/512x512-logos/imgur.png',
...
module.exports = exports = ImgurUserProfileTranslator;
