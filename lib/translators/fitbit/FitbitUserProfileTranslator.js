...
function FitbitUserProfileTranslator() {
}

FitbitUserProfileTranslator.prototype.getMatchPatterns = function() {
    return [ '^acct:fitbit:[A-Za-z0-9]+', '/user/[A-Za-z0-9]+' ];
}
FitbitUserProfileTranslator.prototype.translate = function( uri, owner, rawDoc, callback ) {
	var userProfile = JSON.parse( rawDoc.data ),
		parseAccountId = uri.match( /acct:fitbit:[A-Za-z0-9]+/ ),
	...
	async.eachSeries( 
		...
		function( each, done ) {
			if( userProfile.hasOwnProperty( each ) && parseId[2] ==  userProfile[each].data.encodedId ) {
			try {
				...
				var result = { 
					...
					'id': uri,
					'displayName': {'formatted': userProfile[each].data.displayName },

					// Additional required fields for UI
					'preferredUsername': userProfile[each].data.nickname,
					'thumbnailUrl': userProfile[each].data.avatar,
					'appData': { 
						'serviceName': 'Fitbit',
						// Reuse the icon you defined in LoginStrategies.js.
						'serviceImgUrl': '/images/512x512-logos/fitbit.png',
						// Plus other app data that isn't required.
						'verified': userProfile[each].verified,
					},

					// Everything else
					'location': userProfile[each].data.locale,
					'aboutMe': userProfile[each].data.aboutMe,
					'urls': [ 'null' ],
...
module.exports = exports = FitbitUserProfileTranslator;
