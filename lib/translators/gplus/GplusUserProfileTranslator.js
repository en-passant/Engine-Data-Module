...
function GplusUserProfileTranslator() {
}

GplusUserProfileTranslator.prototype.getMatchPatterns = function() {
    return [ '^acct:gplus:[0-9]+', '/user/[0-9]+' ];
}
GplusUserProfileTranslator.prototype.translate = function( uri, owner, rawDoc, callback ) {
	var userProfile = JSON.parse( rawDoc.data ),
		parseAccountId = uri.match( /acct:gplus:[0-9]+/ ),
		...
		function( each, done ) {
			...
			try {
				...
				var result = { 
					...
					'preferredUsername': userProfile[each].data.displayName,
					'thumbnailUrl': userProfile[each].data.image.url,
					'appData': { 
						'serviceName': 'Google Plus',
						// Reuse the icon you defined in LoginStrategies.js.
						'serviceImgUrl': '/images/512x512-logos/gplus.png',
					...
					// Everything else
					'location': 'null',
					'aboutMe': userProfile[each].data.tagline,
					'urls': [ userProfile[each].data.url ],
					'utcOffset': 'null',
...
module.exports = exports = GplusUserProfileTranslator;
