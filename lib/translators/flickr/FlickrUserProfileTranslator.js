...
function FlickrUserProfileTranslator() {
}

FlickrUserProfileTranslator.prototype.getMatchPatterns = function() {
    return [ '^acct:flickr:[A-Za-z0-9\@]+', '/user/[A-Za-z0-9\@]+' ];
}
FlickrUserProfileTranslator.prototype.translate = function( uri, owner, rawDoc, callback ) {
	var userProfile = JSON.parse( rawDoc.data ),
		parseAccountId = uri.match( /acct:flickr:[A-Za-z0-9\@]+/ ),
		...
		function( each, done ) {
			...
			try {
				...
				var result = { 
					...
					'id': uri,
					'displayName': {'formatted': userProfile[each].data.username._content },

					// Additional required fields for UI
					'preferredUsername': userProfile[each].data.username._content,
					...
					// Everything else
					'location': userProfile[each].data.location._content,
					'aboutMe': 'null',
					'urls': [ userProfile[each].data.photosurl._content ],
					'utcOffset': 'null',
...
module.exports = exports = FlickrUserProfileTranslator;
