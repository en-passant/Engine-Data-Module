...
function RunkeeperUserProfileTranslator() {
}

RunkeeperUserProfileTranslator.prototype.getMatchPatterns = function() {
    return [ '^acct:runkeeper:[0-9]+', '/user/[0-9]+' ];
}
RunkeeperUserProfileTranslator.prototype.translate = function( uri, owner, rawDoc, callback ) {
	var userProfile = JSON.parse( rawDoc.data ),
		parseAccountId = uri.match( /acct:runkeeper:[0-9]+/ ),
	...
	async.eachSeries( 
		...
		function( each, done ) {
			if( userProfile.hasOwnProperty( each ) && parseId[0] ==  userProfile[each].data.userID ) {
			try {
				...
				var result = { 
					...
					'id': uri,
					'displayName': {'formatted': userProfile[each].data.profile.name },

					// Additional required fields for UI
					'preferredUsername': userProfile[each].data.profile.name,
					'thumbnailUrl': '/images/generic_user_image.jpeg',
					'appData': { 
						'serviceName': 'Runkeeper',
						// Reuse the icon you defined in LoginStrategies.js.
						'serviceImgUrl': '/images/512x512-logos/runkeeper.png',
						// Plus other app data that isn't required.
						'verified': userProfile[each].verified,
					},

					// Everything else
					'location': userProfile[each].data.profile.location,
					'aboutMe': 'null',
					'urls': [ userProfile[each].data.profile.profile ],
					'utcOffset': 'null',
...
module.exports = exports = RunkeeperUserProfileTranslator;
